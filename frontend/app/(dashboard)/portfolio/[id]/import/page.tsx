        ...
        <CardContent className="space-y-3">
          {rows.map((row) => (
            <div key={row.id} className="space-y-1">
              <div className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 items-start">
                ...
                <div className="relative">
                  <Input
                    ...
                    onChange={(e) => {
                      updateRow(row.id, { searchQuery: e.target.value, symbol: '' })
                    }}
                  ...
                  {row.symbol && (
                    <p className="text-xs text-primary mt-1">Selected: {row.symbol}</p>
                  )}
                </div>
              ...
              {row.error && (
                <p className="text-xs text-destructive col-span-4">{row.error}</p>
              )}
            </div>
          ))}
